...
### Deterministic financial identifiers

Invoice and receipt identifiers include the complete underlying appointment/payment UUID, with hyphens removed, rather than a shortened UUID prefix. This makes uniqueness deterministic instead of probabilistic while keeping a human-readable prefix/date.

Examples follow the shape:

```text
INV-YYYYMMDD-<FULL_APPOINTMENT_UUID>
RCT-YYYYMMDD-<FULL_PAYMENT_INTENT_UUID>
```
...
A PostgreSQL `BEFORE INSERT` trigger derives the receipt number from the complete payment-intent UUID, so uniqueness does not depend on a truncated identifier generated by an application process.
...
`REFUNDED` means the complete collected amount has been refunded. A PostgreSQL guard prevents a partial refund from presenting the invoice as fully refunded. If `amountRefundedMinor < amountPaidMinor`, the previous paid/partially-paid invoice state is retained. Only once cumulative refunds reach the collected amount may the invoice remain `REFUNDED`.
...
20260907023300_financial_identifier_and_refund_hardening
```
...
2. invoice number uses the complete appointment UUID;
3. cancelled unpaid booking invoice is `VOID`;
4. changing the live service price does not change the historical snapshot/invoice;
5. patient billing ownership works;
6. insurance internal policy reference is not emitted by normal coverage APIs;
7. eligibility allocates patient/insurer responsibility correctly;
8. prior authorization lifecycle is reachable through the treating provider;
9. payment settles through the mock PSP;
10. payment retry with the same idempotency key returns the same settlement;
11. prohibited card credential fields do not exist in persisted payment intent data;
12. receipt number uses the complete payment-intent UUID;
13. invoice transitions to paid state;
14. provider finance shows the correct booking state and balance;
15. finance operator payout creates the expected ledger debit;
16. a partial refund settles and remains idempotent;
17. a partial refund does not falsely mark the invoice fully `REFUNDED`;
18. the remaining refund settles and moves the invoice to `REFUNDED` only after the complete collected amount has been returned;
19. final refund retry is idempotent;
20. ledger includes charge/refund/payout entries;
21. patient cannot access provider financial APIs.
