...
const copy: Record<Locale, {
  ...
  en: {
    ...
    mfaEyebrow: "MULTI-FACTOR VERIFICATION", mfaTitle: "Verify your authenticator", mfaText: "Enter the current six-digit code from your authenticator app.", code: "Verification code", verify: "Verify and continue", verifying: "Verifying…", back: "Use a different account",
    invalid: "Authentication failed. Check your credentials or verification code.", adminOnly: "This portal is restricted to active CarePoint administrators.", rateLimited: "Too many attempts. Please try again later.", unavailable: "CarePoint authentication is temporarily unavailable.", protected: "Protected by revocable sessions and MFA-ready access controls.",
  },
  ar: {
    ...
    mfaEyebrow: "التحقق متعدد العوامل", mfaTitle: "تحقق من تطبيق المصادقة", mfaText: "أدخل الرمز الحالي المكون من ستة أرقام من تطبيق المصادقة.", code: "رمز التحقق", verify: "تحقق وتابع", verifying: "جارٍ التحقق…", back: "استخدام حساب مختلف",
    invalid: "فشل التحقق. راجع بيانات الدخول أو رمز التحقق.", adminOnly: "هذه البوابة مخصصة لمسؤولي CarePoint النشطين فقط.", rateLimited: "محاولات كثيرة جداً. حاول لاحقاً.", unavailable: "خدمة مصادقة CarePoint غير متاحة مؤقتاً.", protected: "محمي بجلسات قابلة للإلغاء وضوابط وصول تدعم MFA.",
  },
  fr: {
    ...
    mfaEyebrow: "VÉRIFICATION MULTIFACTEUR", mfaTitle: "Vérifiez votre authentificateur", mfaText: "Saisissez le code actuel à six chiffres de votre application d’authentification.", code: "Code de vérification", verify: "Vérifier et continuer", verifying: "Vérification…", back: "Utiliser un autre compte",
    invalid: "Échec de l’authentification. Vérifiez vos identifiants ou votre code.", adminOnly: "Ce portail est réservé aux administrateurs CarePoint actifs.", rateLimited: "Trop de tentatives. Réessayez plus tard.", unavailable: "L’authentification CarePoint est temporairement indisponible.", protected: "Protégé par des sessions révocables et des contrôles d’accès compatibles MFA.",
  },
  es: {
      ...
      <section className="admin-login-card">
        ...
        {!challengeId ? (
          <form onSubmit={submitCredentials}>
            ...
            <label>
              <span>{c.email}</span>
              <input type="email" autoComplete="username" value={email} onChange={(event) => setEmail(event.target.value)} required maxLength={320} disabled={busy} />
            </label>
            <label>
              <span>{c.password}</span>
              <input type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} required maxLength={512} disabled={busy} />
            </label>
          ...
          <form onSubmit={submitMfa}>
            ...
            <label>
              <span>{c.code}</span>
              <input className="admin-mfa-code" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} required disabled={busy} autoFocus />
            </label>
            {error ? <div className="admin-login-error" role="alert">{error}</div> : null}
            <button className="admin-login-primary" type="submit" disabled={busy || code.length !== 6}>{busy ? c.verifying : c.verify}</button>
            <button className="admin-login-link" type="button" disabled={busy} onClick={() => { setChallengeId(null); setCode(""); setError(""); }}>{c.back}</button>
          </form>
        )}
      </section>
    </main>
...
function messageForStatus(status: number, c: typeof copy.en): string {
  if (status === 401 || status === 400) return c.invalid;
