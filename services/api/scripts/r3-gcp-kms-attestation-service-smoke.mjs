...
try {
  ...
  delete process.env.CAREPOINT_DOCUMENT_SIGNING_KEY_REF;
  const service = new DocumentsAttestationService();

  await assert.rejects(
    () => service.attest("material"),
    /CAREPOINT_DOCUMENT_SIGNING_KEY_REF is required/,
    "GCP production must select the GCP Cloud KMS signing path before AWS KMS",
  );

  process.env.DOCUMENT_SIGNING_PROVIDER = "oci-vault-kms";
  await assert.rejects(
    () => service.attest("material"),
    /requires GCP Cloud KMS document signing/,
    "GCP production must reject an explicit OCI signing override",
  );
  ...
  await assert.rejects(
    () => service.attest("material"),
    /requires GCP Cloud KMS document signing/,
    "GCP production must reject an explicit AWS signing override",
  );

  process.env.NODE_ENV = "development";
  process.env.CAREPOINT_CLOUD_PROVIDER = "aws";
  process.env.DOCUMENT_SIGNING_PROVIDER = "local";
  process.env.DOCUMENT_SIGNING_SECRET_BASE64 = Buffer.alloc(32, 9).toString("base64");
...
console.log("R3 GCP Cloud KMS document attestation service selection smoke passed");
