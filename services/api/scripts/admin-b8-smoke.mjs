...
const patientEmail = "b8-telehealth-patient-ci@carepoint.test";
const patientPassword = "CarePoint-B8-Patient#2026";
const roomSecretPrefix = "cp_b8_secret_room_";
...
async function cleanup(){
  const patient=await prisma.user.findUnique({where:{email:patientEmail},select:{id:true,patientProfile:{select:{id:true}}}}).catch(()=>null);
...
try {
  await cleanup();
  const registration=await api("/iam/register/patient",{method:"POST",body:JSON.stringify({email:patientEmail,password:patientPassword,firstName:"B8 Secret",lastName:"Tele Patient"})});
  ...
  const patient=await prisma.user.findUnique({where:{email:patientEmail},select:{id:true,patientProfile:{select:{id:true}}}});
  ...
  assert(!snapshotText.includes(patientEmail)&&!snapshotText.includes(patient.id)&&!snapshotText.includes(patient.patientProfile.id),"B8 workspace leaked patient identity.");
  assert(!snapshotText.includes(roomSecretPrefix)&&!snapshotText.includes("b8-hidden-key")&&!snapshotText.includes("b8-hidden-ciphertext"),"B8 workspace leaked room/encryption material.");
  ...
  for(const result of [reset.payload,terminate.payload]){
    ...
    assert(!text.includes(patientEmail)&&!text.includes(patient.id)&&!text.includes(patient.patientProfile.id),"B8 action response leaked patient identity.");
    assert(!text.includes(roomSecretPrefix)&&!text.includes("e2ee"),"B8 action response leaked room/encryption material.");
  }
