...
assert.match(service, /Buffer\.from\(bytes\)\.toString\("utf8"\)/);
...
assert.match(moduleSource, /@Controller\("patient\/clinical-summary-shares"\)/);
assert.doesNotMatch(moduleSource, /@Controller\("patient\/clinical-shares"\)/);
