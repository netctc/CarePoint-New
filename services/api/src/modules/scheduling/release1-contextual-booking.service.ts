...
type VisitContextCreate = {
  ...
  latitude: number; longitude: number; instructions?: string; contactPhone?: string; contactConfirmedAt?: Date; addressValidatedAt?: Date;
};
...
export class Release1ContextualBookingService {
  ...
  async book(principal: AuthPrincipal, input: Release1BookingInput) {
    ...
    if (!input || typeof input !== "object" || Array.isArray(input)) throw new BadRequestException("A booking object is required.");
    ...
    if (existing) {
      ...
      await assertAvailabilityReplay(this.prisma, patient.id, existing.id, input);
      ...
      try {
        const result = await this.prisma.$transaction(async (tx) => {
          ...
          if (duplicate) {
            ...
            await assertAvailabilityReplay(tx, patient.id, duplicate.id, input);
          ...
          // Bind the optional F3 request and its immutable retry payload in this
          // transaction, before capacity changes. Ordinary F1 bookings still work.
          await prepareAvailabilityBooking(tx, patient.id, input, slot);
          if (input.availabilityRequestId != null) {
            const exception = await tx.availabilityException.findFirst({ where: { providerId: slot.providerId, active: true, startsAt: { lt: slot.endsAt }, endsAt: { gt: slot.startsAt }, AND: [{ OR: [{ serviceId: null }, { serviceId: slot.serviceId }] }, { OR: [{ modality: null }, { modality: slot.modality }] }] }, select: { id: true } });
            if (exception) throw new ConflictException("The selected time is unavailable.");
          }
        ...
        if (this.prismaError(error, "P2002")) {
          ...
          if (duplicate?.patientId === patient.id) {
            await assertAvailabilityReplay(this.prisma, patient.id, duplicate.id, input);
            return this.presentAppointment(duplicate.id);
          }
  ...
  async listPatientAppointments(principal: AuthPrincipal) { return this.enrichAppointments(await this.scheduling.listPatientAppointments(principal)); }
  async listProviderAppointments(principal: AuthPrincipal, input: { from?: string; to?: string }) { return this.enrichAppointments(await this.scheduling.listProviderAppointments(principal, input)); }
  private strictClinicContextRequired() { return process.env.NODE_ENV === "production" || process.env.RELEASE1_TEST_STRICT_VISIT_CONTEXT === "true"; }
  ...
  private async clinicContext(tx: Prisma.TransactionClient, serviceId: string, providerId: string): Promise<VisitContextCreate | null> {
    ...
    return { modality: "CLINIC", sourceProviderLocationId: location.id, addressLine1: location.addressLine1, ...(location.addressLine2 ? { addressLine2: location.addressLine2 } : {}), city: location.city, ...(location.region ? { region: location.region } : {}), ...(location.postalCode ? { postalCode: location.postalCode } : {}), countryCode: location.countryCode, latitude: Number(location.latitude), longitude: Number(location.longitude), instructions, addressValidatedAt: location.addressValidatedAt };
  }
  private async homeVisitContext(tx: Prisma.TransactionClient, serviceId: string, homeVisit?: HomeVisitBookingInput): Promise<VisitContextCreate> {
    ...
    const city = this.text(homeVisit?.city, 1, 120, "homeVisit.city"), countryCode = this.country(homeVisit?.countryCode);
    const latitude = this.latitude(homeVisit?.latitude), longitude = this.longitude(homeVisit?.longitude);
    ...
    return { modality: "HOME_VISIT", addressLine1, ...(addressLine2 ? { addressLine2 } : {}), city, ...(region ? { region } : {}), ...(postalCode ? { postalCode } : {}), countryCode, latitude, longitude, ...(instructions ? { instructions } : {}), contactPhone, contactConfirmedAt: new Date(), addressValidatedAt: new Date() };
  }
