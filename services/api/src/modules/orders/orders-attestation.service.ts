...
export class OrdersAttestationService {
  ...
  async attest(value: unknown): Promise<ClinicalAttestation> {
    ...
    if (provider === "aws-kms-hmac") {
      ...
      const result = await this.kmsClient().send(new GenerateMacCommand({ KeyId: keyId, MacAlgorithm: "HMAC_SHA_256", Message: Buffer.from(message, "utf8") }));
      if (!result.Mac) throw new InternalServerErrorException("AWS KMS did not return a clinical order attestation MAC.");
      return { payloadDigest, algorithm: "AWS-KMS-HMAC-SHA256", keyId: result.KeyId ?? keyId, signature: Buffer.from(result.Mac).toString("base64"), signedAt };
    }
  ...
  async verify(value: unknown, input: { payloadDigest: string; signature: string; signedAt: Date; keyId?: string | null; algorithm?: string | null }): Promise<boolean> {
    ...
    if (provider === "aws-kms-hmac") {
      ...
      try {
        const result = await this.kmsClient().send(new VerifyMacCommand({ KeyId: effectiveKeyId, MacAlgorithm: "HMAC_SHA_256", Message: Buffer.from(message, "utf8"), Mac: Buffer.from(input.signature, "base64") }));
  ...
  digest(value: unknown): string { return createHash("sha256").update(JSON.stringify(value)).digest("hex"); }
  private signatureMessage(payloadDigest: string, signedAt: Date): string { return `${payloadDigest}|${signedAt.toISOString()}`; }
  ...
  private provider(): SigningProvider {
    ...
    if (provider !== "local" && provider !== "aws-kms-hmac") throw new InternalServerErrorException(`Unsupported clinical order signing provider '${provider}'.`);
  ...
  private required(name: string): string { const value = process.env[name]?.trim(); if (!value) throw new InternalServerErrorException(`${name} is required.`); return value; }
}
