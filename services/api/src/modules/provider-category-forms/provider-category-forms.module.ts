...
class AdminProviderCategoryFormsController {
  ...
  @RequirePermissions("CATALOG_MANAGE")
  @Post()
  create(@CurrentPrincipal() principal: AuthPrincipal, @Body() body: CreateProviderCategoryFormInput) {
...
class ProviderCategoryFormsController {
  ...
  active(@CurrentPrincipal() principal: AuthPrincipal) {
    return this.forms.activeForProvider(principal);
  }

  @RequirePermissions("PROVIDER_CATEGORY_FORM_SUBMIT")
  @Post(":code/responses")
  @Header("Cache-Control", "no-store")
  submit(
    @CurrentPrincipal() principal: AuthPrincipal,
