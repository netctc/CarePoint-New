...
const GLUCOSE_CONTEXTS = new Set<GlucoseContext>(["FASTING", "PREPRANDIAL", "POSTPRANDIAL", "RANDOM"]);
...
export function isGlucoseMetricCode(value: unknown): boolean {
  ...
  return code === "GLUCOSE" || code.startsWith("GLUCOSE_") || code.endsWith("_GLUCOSE") || code.includes("_GLUCOSE_");
}
...
export function normalizeGlucoseContext(metricCode: unknown, value: unknown): GlucoseContext | null {
  ...
  if (!GLUCOSE_CONTEXTS.has(normalized)) {
    throw new BadRequestException("glucoseContext must be FASTING, PREPRANDIAL, POSTPRANDIAL, or RANDOM.");
  }
  return normalized;
}
