...
export class OtherProviderWorkspaceService {
  ...
  private async clinicalSection(
    ...
    if (section === "HEALTH_PROFILE") {
      const profile = await this.prisma.patientHealthProfile.findUnique({
        where: { patientId },
      });
      ...
      return {
        ...
        items: [{
          ...
          basics: this.object(payload).basics ?? {},
          provenance: {
  ...
  private async observationSection(
    ...
    const consent = await this.currentObservationConsent(
      patientId,
      context.providerId,
      code,
    );
  ...
  private async currentObservationConsent(
    patientId: string,
    providerId: string,
    code: string,
  ) {
